...
import { getUserProfile, UserDataResponse, getUserAddressList, UserAddress } from "../../services/authService";
    ...
    async function loadProfile() {
      try {
        const [profileData, addressData] = await Promise.all([
          getUserProfile(),
          getUserAddressList().catch(() => [] as UserAddress[]),
        ]);
        ...
        <div className="mt-5 rounded-[28px] border border-black/[0.06] bg-white p-6 shadow-[0_8px_30px_rgba(0,39,25,0.06)]">
          <div className="flex items-center justify-between mb-5">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-xs font-bold text-white shadow-[0_4px_16px_rgba(0,105,65,0.22)] transition hover:bg-primary-dim hover:-translate-y-0.5"
            >
          ...
          {addresses.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-center">
              <div className="flex h-16 w-16 items-center justify-center rounded-full bg-primary/5 text-primary/40 mb-4">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-8 w-8">
                  <path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z" />
                  <circle cx="12" cy="10" r="3" />
                </svg>
              </div>
              <p className="text-sm font-bold text-black/40">No addresses saved yet</p>
              <p className="mt-1 text-xs font-medium text-black/30">Tap the button above to add your first delivery address</p>
            </div>
          ) : (
            <div className="flex flex-col gap-3">
              {addresses.map((addr) => (
                <div
                  key={addr.addressId}
                  className="group rounded-2xl border border-black/[0.06] bg-[#fafcfb] p-5 transition hover:border-primary/20 hover:bg-white hover:shadow-[0_4px_16px_rgba(0,105,65,0.06)]"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-xs font-semibold text-black/50">{addr.recipientPhoneNumber}</p>
                      ...
                      {addr.notes && (
                        <p className="mt-2 text-xs font-medium text-black/35 italic">
                          Note: {addr.notes}
                        </p>
                      )}
                    </div>
